...
public class NarGnuConfigureMojo extends AbstractGnuMojo {
     ...
     * 
     * @parameter expression="${nar.gnu.configure.args}"
     */
    private List<String> gnuConfigureArgs;

    /**
     * Arguments to pass to GNU configure. Space separated arguments list of arguments.
     * Will work in combination with &lt;gnuConfigureArgs&gt;.
     * 
     * @parameter expression="${nar.gnu.configure.argSet}" default-value=""
     */
    private String gnuConfigureArgSet;
	...
	public final void narExecute() throws MojoExecutionException,
		...
		if (getGnuSourceDirectory().exists()) {
			...
			if (!gnuConfigureSkip && configure.exists()) {
                ...
                argsList.add("./" + configure.getName());
                argsList.add("--prefix="
                        + getGnuAOLTargetDirectory().getAbsolutePath());

                if (gnuConfigureArgs != null) {
                    for (String arg : gnuConfigureArgs) {
                        argsList.add(arg);
                    }
				}

                if (gnuConfigureArgSet != null) {
                    final String[] argSet = gnuConfigureArgSet.split("\\s");
                    for (int i = 0; i < argSet.length ; i++) {
                        argsList.add(argSet[i]);
                    }
                }
