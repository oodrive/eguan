...
import com.oodrive.nuage.utils.unix.UnixFsFile;
import com.oodrive.nuage.utils.unix.UnixMount;
import com.oodrive.nuage.utils.unix.UnixUser;
...
public final class TestFiles {
    ...
    public void testDeleteRecursive() throws IOException {
        ...
        // Delete and check
        com.oodrive.nuage.utils.Files.deleteRecursive(tempD1);
    ...
    public void testDeleteRecursiveNotExisting() throws IOException {
        ...
        com.oodrive.nuage.utils.Files.deleteRecursive(tempFile.toPath());
    ...
    public void testDeleteRecursiveSymbolicLink() throws IOException {
        ...
        try {
            ...
            com.oodrive.nuage.utils.Files.deleteRecursive(tempLink);
    ...
    private void deleteRecursiveProgress(final boolean keepPath) throws IOException {
        ...
        try {
            ...
            // Delete and check
            com.oodrive.nuage.utils.Files.deleteRecursive(tempD1, keepPath,
                    new com.oodrive.nuage.utils.Files.DeleteRecursiveProgress() {
    ...
    public void testDeleteRecursiveProgressNotExisting() throws IOException {
        final File tempFile = File.createTempFile(TEMP_PREFIX, TEMP_SUFFIX);
        Assert.assertTrue(tempFile.delete());
        com.oodrive.nuage.utils.Files.deleteRecursive(tempFile.toPath(), false,
                new com.oodrive.nuage.utils.Files.DeleteRecursiveProgress() {
                    @Override
                    public final FileVisitResult notify(final Path path) {
                        return FileVisitResult.CONTINUE;
    ...
    private void deleteRecursiveProgressSingle(final Path toDel, final boolean keepPath, final boolean cleanup)
        ...
        try {
            // Delete and check
            com.oodrive.nuage.utils.Files.deleteRecursive(toDel, keepPath,
                    new com.oodrive.nuage.utils.Files.DeleteRecursiveProgress() {
            ...
            // Cleanup remaining elements
            if (cleanup)
                com.oodrive.nuage.utils.Files.deleteRecursive(toDel);
        }
    }
    ...
    public void testFileExistsTimeout() throws IOException {
        final File tempFile = File.createTempFile(TEMP_PREFIX, TEMP_SUFFIX);
        try {
            Assert.assertTrue(com.oodrive.nuage.utils.Files.waitForFile(tempFile, 100));
        }
        finally {
            Assert.assertTrue(tempFile.delete());
        }
        Assert.assertFalse(com.oodrive.nuage.utils.Files.waitForFile(tempFile, 100));
    }
    ...
    public void testFileNotExistsTimeout() throws IOException {
        final File tempFile = File.createTempFile(TEMP_PREFIX, TEMP_SUFFIX);
        try {
            Assert.assertFalse(com.oodrive.nuage.utils.Files.waitForFileDeletion(tempFile, 100));
        }
        finally {
            Assert.assertTrue(tempFile.delete());
        }
        Assert.assertTrue(com.oodrive.nuage.utils.Files.waitForFileDeletion(tempFile, 100));
    }
    ...
    public void userAttrExt2() throws IOException {
        ...
        { // Create file and check attr supported
            ...
            try {
                ...
                Assert.assertEquals(0, com.oodrive.nuage.utils.Files.listUserAttr(aaPath).length);
                com.oodrive.nuage.utils.Files.checkUserAttrSupported(aaPath);

                // Set an attribute
                com.oodrive.nuage.utils.Files.setUserAttr(aaPath, ATTR1);
                assertTrue(com.oodrive.nuage.utils.Files.isUserAttrSet(aaPath, ATTR1));
                Assert.assertEquals("y", com.oodrive.nuage.utils.Files.getUserAttr(aaPath, ATTR1));

                com.oodrive.nuage.utils.Files.setUserAttr(aaPath, ATTR2, "");
                assertTrue(com.oodrive.nuage.utils.Files.isUserAttrSet(aaPath, ATTR2));
                Assert.assertEquals("", com.oodrive.nuage.utils.Files.getUserAttr(aaPath, ATTR2));

                com.oodrive.nuage.utils.Files.setUserAttr(aaPath, ATTR2, VALUE2);
                assertTrue(com.oodrive.nuage.utils.Files.isUserAttrSet(aaPath, ATTR2));
                Assert.assertEquals(VALUE2, com.oodrive.nuage.utils.Files.getUserAttr(aaPath, ATTR2));

                Assert.assertEquals(2, com.oodrive.nuage.utils.Files.listUserAttr(aaPath).length);
                com.oodrive.nuage.utils.Files.unsetUserAttr(aaPath, ATTR1);
                Assert.assertEquals(1, com.oodrive.nuage.utils.Files.listUserAttr(aaPath).length);
                Assert.assertArrayEquals(new String[] { ATTR2 }, com.oodrive.nuage.utils.Files.listUserAttr(aaPath));

                // Does nothing if the attribute is not set
                com.oodrive.nuage.utils.Files.unsetUserAttr(aaPath, ATTR1);

                assertFalse(com.oodrive.nuage.utils.Files.isUserAttrSet(aaPath, ATTR1));
                assertTrue(com.oodrive.nuage.utils.Files.isUserAttrSet(aaPath, ATTR2));
                assertFalse(com.oodrive.nuage.utils.Files.isUserAttrSet(aaPath, ATTR3));
                Assert.assertNull(com.oodrive.nuage.utils.Files.getUserAttr(aaPath, ATTR3));
            }
            finally {
                unixMount.umount();
            }
        ...
        { // Mount FS read-only and check attr not supported
            ...
            try {
                ...
                assertTrue(com.oodrive.nuage.utils.Files.isUserAttrSet(aaPath, ATTR2));
                assertFalse(com.oodrive.nuage.utils.Files.isUserAttrSet(aaPath, ATTR3));
                Assert.assertNull(com.oodrive.nuage.utils.Files.getUserAttr(aaPath, ATTR1));
                Assert.assertEquals(VALUE2, com.oodrive.nuage.utils.Files.getUserAttr(aaPath, ATTR2));
                Assert.assertNull(com.oodrive.nuage.utils.Files.getUserAttr(aaPath, ATTR3));
                ...
                // Can not delete
                try {
                    com.oodrive.nuage.utils.Files.unsetUserAttr(aaPath, ATTR2);
                    throw new AssertionFailedError("Should not be reached");
                }
                catch (final IOException e) {
                    // Ok
                ...
                // Can not set
                try {
                    com.oodrive.nuage.utils.Files.setUserAttr(aaPath, ATTR3, "");
                    throw new AssertionFailedError("Should not be reached");
                }
                catch (final IOException e) {
                    // Ok
    ...
    public void userAttrExt2NoAccess() throws IOException {
        ...
        try {
            ...
            com.oodrive.nuage.utils.Files.checkUserAttrSupported(lf.toPath());
        }
        finally {
            unixMount.umount();
        }
    ...
    public void userAttrVfat() throws IOException {
        ...
        try {
            ...
            try {
                com.oodrive.nuage.utils.Files.checkUserAttrSupported(tf.toPath());
                throw new AssertionFailedError("Should not be reached");
            }
            catch (final IOException e) {
                // Ok
