...
public abstract class TestNrsFileJanitorAbstract extends AbstractNrsTestFixture {
    ...
    protected final NrsFile createTestNrsFile(final NrsFileJanitor janitor, final MetaConfiguration config,
            final boolean addFlags, final boolean flagBlock) throws NrsException {
        return createTestNrsFile(janitor, config, addFlags, true, false, flagBlock);
    }

    protected final NrsFile createTestNrsFile(final NrsFileJanitor janitor, final MetaConfiguration config,
            final boolean addFlags, final boolean random, final boolean flagRoot, final boolean flagBlock)
            throws NrsException {
        // Check cluster size
        ...
        if (addFlags || flagBlock)
            return janitor.createNrsFile(header);
        else
            return janitor.createNrsFile(header, flagRoot ? NrsFileFlag.ROOT : NrsFileFlag.PARTIAL);
    }
