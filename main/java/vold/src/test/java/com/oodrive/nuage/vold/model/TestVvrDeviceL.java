...
public class TestVvrDeviceL extends TestVvrManagerAbstract {
    ...
    public void testSetIqn() {
        ...
        device.setIqn("iqn.2000-06.com.oodrive.nuage:test1");
        voldTestHelper.waitTaskEnd(vvrUuid, device.activateRO(), dummyMbeanServer);

        try {
            device.setIqn("iqn.2000-06.com.oodrive.nuage:test2");
        }
        catch (final IllegalStateException e) {
            exceptionOnRO = true;
        }
        ...
        try {
            voldTestHelper.waitTaskEnd(vvrUuid, device.setIqnNoWait("iqn.2000-06.com.oodrive.nuage:test2"),
                    dummyMbeanServer);
        }
        catch (final IllegalStateException e) {
            exceptionOnRW = true;
