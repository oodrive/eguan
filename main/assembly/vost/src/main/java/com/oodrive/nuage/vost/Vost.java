...
import com.oodrive.nuage.rest.container.JettyConfigurationContext;
import com.oodrive.nuage.rest.container.ServerAddressConfigKey;
import com.oodrive.nuage.rest.container.ServerPortConfigKey;
import com.oodrive.nuage.utils.LogUtils;
...
import com.oodrive.nuage.vold.adm.RestLauncher;

public final class Vost {

    private static final Logger LOGGER = LoggerFactory.getLogger(Vost.class);

    /**
     * VOST shutdown hook.
     * 
     */
    class VostShutdownHook implements Runnable {
    ...
    /** No termination */
    private static final int VOST_OK = -1;
    /** Vost already cancelled */
    private static final int VOST_CANCELLED = -2;
    ...
    public static void main(final String[] args) {
        ...
        Thread.setDefaultUncaughtExceptionHandler(new VostUncaughtExceptionHandler());

        // Configure SysLog appender for logback
        LogUtils.initSysLog();

        final Vost vost = new Vost();
        ...
        // Shutdown hook
        final Thread hook = new Thread(vost.new VostShutdownHook(), "VOST shutdown hook");
        Runtime.getRuntime().addShutdownHook(hook);

        // Start VOLD
        int status = vost.startVold(voldDir);

        if (status == VOST_CANCELLED) {
            return;
        }
        else if (status != VOST_OK) {
            System.exit(status);
        }

        // Start REST
        status = vost.startRestServer(args[1]);
        ...
        if (cinderCmd == null) {
            ...
            status = vost.waitRestServer();
        }
        else {
            // Start Cinder
            status = vost.startCinder(cinderCmd);
    ...
    private final int startRestServer(final String configPath) {
        ...
        try (final FileInputStream configInputStream = new FileInputStream(configPath);
                final InputStream contextInputStream = Vost.class.getResourceAsStream(DEFAULT_CONTEXT_RESOURCE);
    ...
    private final int startCinder(final String[] cinderCmd) {
        synchronized (cancelled) {
            ...
            cinder = RunCmdUtils.newRunningCmd(cinderCmd, Vost.class);
        }
        return waitCinder();
    }
