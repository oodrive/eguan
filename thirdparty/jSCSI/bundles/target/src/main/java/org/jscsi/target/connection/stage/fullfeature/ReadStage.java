...
public class ReadStage extends ReadOrWriteStage {
    ...
    public void execute(ProtocolDataUnit pdu) throws IOException, InterruptedException, InternetSCSIException,
        ...
        try {
            ...
            // TODO: Send check condition to the initiator, and then when the initiator will request MODE SENSE, 
            // return a response with Medium Error - unrecovered read error (see http://en.wikipedia.org/wiki/Key_Code_Qualifier)
            throw e;
        }
    }

    private void doExecute(ProtocolDataUnit pdu) throws IOException, InterruptedException,
    InternetSCSIException, SettingsException {
        ...
        final int blockSize = session.getStorageModule().getBlockSize();
        ...
        if (bytesSent < totalTransferLength - settings.getMaxRecvDataSegmentLength()) {
             ...
             */
            //dataSegment = connection.getDataInBuffer(settings.getMaxRecvDataSegmentLength());
            //dataSegment = ByteBuffer.wrap(dataSegmentArray);
        }

        while (bytesSent < totalTransferLength - settings.getMaxRecvDataSegmentLength()) {

            dataSegment = ByteBufferCache.allocate(settings.getMaxRecvDataSegmentLength());

            // get data and prepare data segment
            session.getStorageModule().read(dataSegment, storageOffset + bytesSent);
        ...
        dataSegment = ByteBufferCache.allocate(bytesRemaining);
        session.getStorageModule().read(dataSegment, storageOffset + bytesSent);
        //dataSegment = ByteBuffer.wrap(dataSegmentArray);
