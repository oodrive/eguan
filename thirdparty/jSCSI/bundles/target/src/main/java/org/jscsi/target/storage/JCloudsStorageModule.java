...
public class JCloudsStorageModule implements IStorageModule {
    ...
    public final static int SIZE_PER_BUCKET = BLOCK_IN_CLUSTER * DEFAULT_BLOCK_SIZE;
    ...
    */
    ...
    public synchronized void read(ByteBuffer bytes, long storageIndex) throws IOException {
        ...
        try {
            ...
            final ByteArrayDataOutput output = ByteStreams.newDataOutput(bytes.capacity());
            int length = -1;
            if (bucketOffset + bytes.capacity() > SIZE_PER_BUCKET) {
            ...
            if (bucketOffset + bytes.capacity() > SIZE_PER_BUCKET) {
                ...
                output.write(data, 0, bytes.capacity() - (SIZE_PER_BUCKET - bucketOffset));
            }
            // OODRIVE : de-activate ByteBuffer direct or re-coded with channel
            System.arraycopy(output.toByteArray(), 0, bytes.array(), 0, bytes.capacity());
        } catch (Exception exc) {
    ...
    public synchronized void write(ByteBuffer bytes, long storageIndex) throws IOException {
        ...
        try {
            ...
            System.arraycopy(bytes, 0, data, bucketOffset, bytes.capacity() + bucketOffset > SIZE_PER_BUCKET
                ? SIZE_PER_BUCKET - bucketOffset : bytes.capacity());
            ...
            if (bucketOffset + bytes.capacity() > SIZE_PER_BUCKET) {
                ...
                System.arraycopy(bytes.array(), SIZE_PER_BUCKET - bucketOffset, data, 0, bytes.capacity()
